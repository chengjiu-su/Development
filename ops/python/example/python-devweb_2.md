...
- 继承性：父传子，子传孙
- 层叠性：一个元素可以应用多个样式表，不冲突时，所有样式均生效
- 优先级：如果应用的多个样式有冲突，样式表有优先级。子元素样式优先；后定义的样式优先。

#### 选择器：选择为谁设置样式

- \*是通用选择器，可以匹配所有的元素
- 元素选择器：HTML的标签天然就是选择器
- 类选择器：可以为元素设置名为class的属性，多个元素可以属于相同的class，为它们统一设置样式
- 类选择器可以与其他元素共同使用
  - p.c2：表示p标签有一个c2属性\<p class="c2">sssss\<p>
  - p .c3：表示p元素中嵌套了c3，即\<p>xxxxx\<span class="c3">yyyyy\</span>zzzzz\</p>
- id选择器：为某个元素添加的唯一标识
- 群组选择器：使用逗号将多个选择器分隔，统一设置样式
- 伪类选择器：经常用于为超链接设置访问前(历史记录中没有)、鼠标悬停和访问后的样式

#### 尺寸

- 可以使用百分比，如某一个块级元素占全页面的50％
- px指像素，就是屏幕上的一个点
- em：相对大小，如1.2em指的是父元素的1.2倍
- rem：相对大小，如1.2em指的是根元素的1.2倍

#### 颜色

- 使用RGB三原色：Red / Green / Blue
- 三种颜色，每种颜色都使用8位2进制数字表示
- 某一颜色数值越大，表示该颜色越亮；否则越暗
...
- 在html中，一切皆框
- 主要用于确定某一元素在页面中占多少空间
- 一般设置宽度，不设置高度。因为高度是由页面内容多少决定的
- 元素所占宽度是：元素宽度＋左右内边距＋左右边框＋左右外边距

## bootstrap

- bootstrap是twitter公司开发的开源前端web框加
- 可以简单的理解为它是一个大样式表。
- 中文官方站：http://bootcss.com


- 准备环境，将18/devweb/day02/目录的static目录拷贝到工作目录
